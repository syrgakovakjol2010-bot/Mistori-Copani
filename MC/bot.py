...
# Твой ID (цифрами). Замени на свой, если он другой!
# Чтобы узнать точно, напиши боту /start — он выведет ID в консоль.
ADMIN_ID = 5385396977
...
def start(message):
    # Выводим ID в консоль (для тебя, чтобы узнать свой номер)
    print(f"User ID: {message.chat.id}")
    
    # Если пишет Админ
    if message.chat.id == ADMIN_ID:
        bot.send_message(message.chat.id, f"Привет, Шеф! Твой ID: {message.chat.id}")
...
def callback_admin(call):
    if call.data.startswith("connect_"):
        ...
        # Убираем кнопку, чтобы не нажать второй раз
...
def chat_logic(message):
    ...
    if "цена" in text or "прайс" in text or "сколько стоит" in text:
        try:
            ...
            bot.send_message(user_id, "Ошибка: Файл прайса не найден (скажите админу).")
